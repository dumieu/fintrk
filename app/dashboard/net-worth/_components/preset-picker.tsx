      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="relative flex w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-chart-border bg-card/95 shadow-2xl"
            style={{ boxShadow: `0 20px 60px -15px ${accent}40, 0 0 0 1px ${accent}20 inset` }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header / search */}
            <div className="flex items-center gap-2 border-b border-chart-border px-4 py-3">
              <span
              ...
              <div className="flex flex-1 items-center gap-2 rounded-lg border border-chart-border bg-chart-muted px-2.5">
                ...
                <input
                  ...
                  className="h-8 w-full bg-transparent text-sm text-foreground placeholder:text-muted-foreground/80 outline-none"
                  autoFocus
                />
                {query && (
                  <button
            ...
            <div ref={listRef} className="max-h-[60vh] overflow-y-auto px-1 py-1">
              {flat.length === 0 && (
                <div className="px-4 py-8 text-center">
                  ...
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[11px] font-semibold transition hover:bg-chart-muted"
              ...
              {flat.length > 0 && query.trim() && (
                <div className="border-t border-chart-border px-2 py-1.5">
                  <button
                    type="button"
                    onClick={() => {
                      onPick({ kind: "custom", categoryId: "other", label: query.trim() });
                    ...
                    className="flex w-full items-center gap-3 rounded-lg px-2.5 py-1.5 text-left text-muted-foreground transition hover:bg-chart-muted"
                  >
                    <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-chart-muted text-muted-foreground">
                      <Plus className="h-3.5 w-3.5" />
                    </span>
                    <span className="flex-1 text-sm">
                      Add &ldquo;<span className="font-semibold text-foreground">{query.trim()}</span>&rdquo; as custom
                    </span>
                    <span className="hidden text-[10px] text-muted-foreground/80 sm:inline">⏎</span>
                  </button>
                </div>
              )}
            </div>
            ...
            {/* Footer hint */}
            <div className="flex items-center justify-between border-t border-chart-border px-4 py-2 text-[10px] text-muted-foreground">
              <span>
                <kbd className="rounded border border-chart-border bg-chart-muted px-1">↑</kbd>{" "}
                <kbd className="rounded border border-chart-border bg-chart-muted px-1">↓</kbd>{" "}
                navigate ·{" "}
                <kbd className="rounded border border-chart-border bg-chart-muted px-1">↵</kbd>{" "}
                add ·{" "}
                <kbd className="rounded border border-chart-border bg-chart-muted px-1">esc</kbd>{" "}
                close
              </span>
